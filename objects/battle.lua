...
function Battle:attack(p1,p2,m)
	...
	if(p2.protected and _moves[m].nature==0)then
	...
	elseif(_moves[m].nature==2) then
		self:type2(p1,m)
	end
	--added reset protected
	p2.protected = false
	--added repeated move
	if(p1.lastmove==m)then
		p1.repeated = p1.repeated+1
	else
		p1.repeated = 1
	end
	p1.lastmove = m
...
function Battle:type2(p1,m)
	chance = 255
	if(p1.lastmove==m)then
		rate = math.max(1/8,(1/(p1.moverepeat+1)))
		chance = chance*rate
	end
...
function Battle:calc()
	...
	print(_moves[self.move2].priority)
	if(_moves[self.move1].priority>_moves[self.move2].priority or (_moves[self.move1].priority==_moves[self.move2].priority and pok1.speed>=pok2.speed))then
